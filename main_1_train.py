...
fold_list = {'train': ['training_images', 'training_images_HoriFlip'],
             'valid': ['training_images']}
data_root = r'/data/S/LinGroup/Users/YC/VRDL_HW1'
save_root = data_root+'/record_'+strftime("%m%d_%H%M", localtime())
...
DS = Prepare_data(MBS, fold_list, train_frame, valid_frame)
train_set, valid_set, train_loader, valid_loader = DS
del DS
...
for ep in range(1, Max_epoch+1):
    ...
    train_acc = np.sum(train_acc_list)/(2400*len(fold_list['train']))
    ...
    valid_acc = np.sum(valid_acc_list)/600
    ...
    print(f"epoch: {ep}/{Max_epoch},\
     train_loss: {train_loss:.4f},\
     train_acc: {train_acc:.4f},\
     val_loss: {valid_loss:.4f},\
     val_acc: {valid_acc:.4f}")
    print(f"total time: {int(time() - tic)},\
     lr: {lr_scheduler.get_last_lr()[0]:.6f}")

    if valid_acc >= save_threshold:
        torch.save(model.module.state_dict(),
                   save_root + f'/ep={ep:03}-acc={valid_acc:.4f}.hdf5')
    if ep % 5 == 0:
        print(f"Current best, Epoch : {max_val_ep_acc[0]:03},\
         Acc : {max_val_ep_acc[1]:.4f}")

    with open(os.path.join(save_root, output_record), 'a') as f:
        f.write(f"epoch: {ep:3d}/{Max_epoch},\
         train_loss: {train_loss:.4f},\
         train_acc: {train_acc:.4f},\
         val_loss: {valid_loss:.4f},\
         val_acc: {valid_acc:.4f}\n")
